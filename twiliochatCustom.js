var twiliochat = (function () {
    ...
    var FRIENDLY_CHANNEL_UNIQUE_NAME = '';
    ...
    $(document).ready(function () {
        //Current Loged in userUserName auto Connect 
        connectClientWithUsername("doctorlogin");
        tc.init();
    });
    ...
    function CreateNewChannel(channelName) {
        ...
        if (!tc.privateChannel) {
            tc.messagingClient
                .createChannel({
                    friendlyName: channelName,
                })
                .then(function (channel) {
                    tc.privateChannel = channel;
                    FRIENDLY_CHANNEL_UNIQUE_NAME = channel.friendlyName;
            ...
            setupChannel(tc.privateChannel);
        }
    }

    function connectClientWithUsername(usernameText) {
    ...
    function fetchAccessToken(username, handler) {
        $.post('/token', { identity: username }, null, 'json')
            // $.post(SiteDomain + '/twillioWebChat/getToken/' + username, null, 'json')
    ...
    function connectMessagingClient(token) {
        ...
        Twilio.Chat.Client.create(token).then(function (client) {
            ...
            //join channel
            //GENERAL_CHANNEL_UNIQUE_NAME = "DocPatCommunication1";
            //GENERAL_CHANNEL_NAME = "docpatcommunication1";
            debugger
            CreateNewChannel("TestNew4");
    ...
    function updateChannelUI(selectedChannel) {
        ...
        if (tc.currentChannelContainer === undefined && selectedChannel.friendlyName === FRIENDLY_CHANNEL_UNIQUE_NAME) {
    ...
    function addChannel(channel) {
        if (channel.uniqueName === FRIENDLY_CHANNEL_UNIQUE_NAME) {
            // tc.generalChannel = channel;
            tc.privateChannel = channel;
        }
